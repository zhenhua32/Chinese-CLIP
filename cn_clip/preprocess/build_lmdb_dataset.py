...
if __name__ == "__main__":
    ...
    # 按 train, valid, test 分别建立 lmdb 文件夹
    for split in specified_splits:
        ...
        pairs_size = os.path.getsize(pairs_annotation_path)
        base64_size = os.path.getsize(base64_path)

        # map_size 有点坑, 这会非常浪费空间的. 所以根据 getsize 来计算 map_size
        lmdb_img = os.path.join(lmdb_split_dir, "imgs")
        env_img = lmdb.open(lmdb_img, map_size=int(base64_size * 1.5))  # 1.5 倍数可能也有点浪费
        txn_img = env_img.begin(write=True)
        lmdb_pairs = os.path.join(lmdb_split_dir, "pairs")
        env_pairs = lmdb.open(lmdb_pairs, map_size=int(pairs_size * 2))  # 这个是纯文本, 倍数大点也没关系
        ...
        with open(pairs_annotation_path, "r", encoding="utf-8") as fin_pairs:
            ...
            for line in tqdm(fin_pairs):
                ...
                for image_id in obj["image_ids"]:
                    # 这边是用 pickle 来序列化的, 所以加载的时候也要用 pickle 反序列化
                    dump = pickle.dumps((image_id, obj["text_id"], obj["text"]))  # encoded (image_id, text_id, text)
                    # 总之 key 和 value 都是 bytes 类型
                    # key 是 write_idx, value 是 pickle 编码的 (image_id, text_id, text)
        ...
        with open(base64_path, "r", encoding="utf-8") as fin_imgs:
            ...
            for line in tqdm(fin_imgs):
                ...
                # key 是 image_id, value 是 base64 编码的图片
